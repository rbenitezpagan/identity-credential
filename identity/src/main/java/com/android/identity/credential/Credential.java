 ...
 * per credential. Additionally, the <en>shape</en> of the credential (for example
 * MDOC as per ISO/IEC 18013-5:2021) and its type (for example <b>org.iso.18013.5.1.mDL</b>,
 * may not exist for all shapes) must also be specified. Finally, for each credential
 * an asymmetric key-pair called <em>CredentialKey</em> is generated and this key-pair
 * is intended to be the primary identifier for the credential and used for communication
 * with the credential issuing authority (IA). In particular, the IA can examine the
 * attestation on <em>CredentialKey</em> to verify that the device is in a known good
 * state (e.g. verified boot is enable, the OS is at a sufficiently recent patch
 * level, it's communicating with the expected Android application, etc.) before
 ...
 * credential creation and when data in the credential has changed.
 *
 ...
 * some shape-specific data related to the key. For MDOCs the authentication key
 * plays the role of <em>DeviceKey</em> and the issuer-signed data includes the
 * <em>Mobile Security Object</em> which includes the authentication key and is
 * signed by the issuer. This is used for anti-cloning and to return data signed
 ...
 * to get a {@link PendingAuthenticationKey}. With this in hand, it should use
 ...
 * to find a suitable {@link Credential} for presentation. The steps that follow
 * depend on the credential shape and presentation protocol, in the following we'll
 * assume ISO/IEC 18013-5:2021 is used. TODO: flesh this paragraph out a bit more.
 */
